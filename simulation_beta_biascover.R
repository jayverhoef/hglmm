...
for(iter in 1:niter) {
	...
	pred_out = estpred(optout$par, y = y, X = X, distmat = distmat, 
		autocor_fun = rho_exp, maxvar = maxvar, maxrange = maxrange, 
		maxphi = maxphi,
		family = 'beta', Xp = Xp, dist_op = dist_op, dist_pp = dist_pp )
  pred_out$betahat
