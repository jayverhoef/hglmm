...
for(iter in 1:niter) {
	...
	sim_data = simSGLM_wExpl(200, autocor_fun = rho_exp, 
		...
		loc_type = 'random', family = 'gamma', pred = TRUE)
	...
	optout = optim(theta, logLik_Laplace, y = y, X = X, distmat = distmat, 
		...
		maxphi = maxphi, family = 'gamma', mlmeth = 'reml')
	# covariance parameters

	source('estpred.R')
	# undebug(estpred)
	pred_out = estpred(optout$par, y = y, X = X, distmat = distmat, 
		autocor_fun = rho_exp, maxvar = maxvar, maxrange = maxrange, maxphi = maxphi,
		family = 'gamma', Xp = Xp, dist_op = dist_op, dist_pp = dist_pp)
