setwd('/mnt/ExtraDrive1/Work/desktop_data/2022_papers/hglmm')
...
betas = c(1, 0.5, -0.5, 0.5)
gammas = c(1, 1, 0.0001, 1)
niter = 2000
...
for(iter in 1:niter) {
	...
	sim_data = simSGLM_wExpl(300, autocor_fun = rho_exp, 
		betas = betas, gammas = gammas, 
		loc_type = 'random', family = 'invgauss', pred = TRUE)
	var(sim_data$y - exp(sim_data$w_true))
	...
	theta = c(rep(-2, times = 2),-5)
	# undebug(logLik_Laplace)
	# optimize for covariance parameters
	maxvar = 10*var(log(y+.0001))
	maxrange = 10*max(distmat)
	maxphi = 100
	source('logLik_Laplace.R')
	# undebug(logLik_Laplace)
	optout = optim(theta, logLik_Laplace, y = y, X = X, distmat = distmat, 
		autocor_fun = rho_sph, maxvar = maxvar, maxrange = maxrange, 
		maxphi = maxphi, family = 'invgauss', mlmeth = 'reml',
		use.nugget = FALSE)
	...
	pred_out = estpred(optout$par, y = y, X = X, distmat = distmat, 
		autocor_fun = rho_sph, maxvar = maxvar, maxrange = maxrange, maxphi = maxphi,
		family = 'invgauss', Xp = Xp, dist_op = dist_op, dist_pp = dist_pp,
		use.nugget = FALSE)
  pred_out$theta_trans
  cbind(pred_out$betahat, sqrt(diag(pred_out$covbeta)), 
		sqrt(diag(pred_out$covbeta_adj)))
